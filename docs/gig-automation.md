...
Image-only scans currently require an operator-selected OCR or manual path before
they can enter this runner. Record that time separately in the delivery log. Do
not describe the runner itself as having native OCR until that path is implemented
and verified.
